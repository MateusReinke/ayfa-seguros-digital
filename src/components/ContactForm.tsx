...
const ContactForm = () => {
  ...
  return (
    <section id="contato" className="py-16 md:py-24 lg:py-32 relative overflow-hidden" ref={ref}>
      {/* Gradient background */}
      <div className="absolute inset-0 bg-gradient-to-br from-background via-muted to-background" />
      <div className="absolute top-0 right-0 w-48 md:w-96 h-48 md:h-96 bg-cyan/10 rounded-full blur-3xl" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 lg:gap-12 max-w-6xl mx-auto">
          ...
          <Card className={`border border-border/50 bg-card shadow-elegant transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            ...
            <CardHeader className="pb-4 md:pb-6 p-4 md:p-6">
              <CardTitle className="font-display text-xl md:text-2xl lg:text-3xl font-bold text-foreground">Envie sua Mensagem</CardTitle>
            </CardHeader>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
                <div>
                  <Input
                    ...
                    required
                    className="border-border h-12 md:h-14 text-sm md:text-base focus:border-primary focus:ring-primary"
                  />
                </div>
                ...
                <div>
                  <Textarea
                    ...
                    required
                    className="min-h-[120px] md:min-h-[160px] border-border text-sm md:text-base focus:border-primary focus:ring-primary"
                  />
                </div>
                
                <Button
                  type="submit"
                  className="w-full bg-gradient-to-r from-accent to-gold hover:opacity-90 text-white h-12 md:h-14 text-base md:text-lg font-semibold shadow-glow hover:scale-105 transition-all duration-300"
                  size="lg"
                >
                  Enviar Solicitação
                </Button>
          ...
          <div className={`space-y-4 md:space-y-6 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`} style={{ transitionDelay: '200ms' }}>
            <Card className="border border-border/50 bg-card hover:shadow-elegant transition-all duration-300 overflow-hidden">
              ...
              <CardContent className="p-5 md:pt-8 md:pb-8 md:px-6">
                <div className="flex items-start gap-4 md:gap-6">
                  <div className="w-12 md:w-16 h-12 md:h-16 rounded-xl md:rounded-2xl bg-cyan flex items-center justify-center flex-shrink-0">
                    <Mail className="w-6 md:w-8 h-6 md:h-8 text-white" />
                  </div>
                  <div>
                    <h3 className="font-bold text-foreground mb-2 md:mb-4 text-lg md:text-xl">Email</h3>
            ...
            <Card className="border border-border/50 bg-card hover:shadow-elegant transition-all duration-300 overflow-hidden">
              ...
              <CardContent className="p-5 md:pt-8 md:pb-8 md:px-6">
                <div className="flex items-start gap-4 md:gap-6">
                  <div className="w-12 md:w-16 h-12 md:h-16 rounded-xl md:rounded-2xl bg-accent flex items-center justify-center flex-shrink-0">
                    <Phone className="w-6 md:w-8 h-6 md:h-8 text-white" />
                  </div>
                  <div>
                    <h3 className="font-bold text-foreground mb-2 md:mb-4 text-lg md:text-xl">Telefone</h3>
            ...
            <Card className="border border-border/50 bg-card hover:shadow-elegant transition-all duration-300 overflow-hidden">
              ...
              <CardContent className="p-5 md:pt-8 md:pb-8 md:px-6">
                <div className="flex items-start gap-4 md:gap-6">
                  <div className="w-12 md:w-16 h-12 md:h-16 rounded-xl md:rounded-2xl bg-purple flex items-center justify-center flex-shrink-0">
                    <MapPin className="w-6 md:w-8 h-6 md:h-8 text-white" />
                  </div>
                  <div>
                    <h3 className="font-bold text-foreground mb-2 md:mb-4 text-lg md:text-xl">Localização</h3>
                    <p className="text-muted-foreground text-sm md:text-base font-medium leading-relaxed">
                      São Paulo - SP<br />
